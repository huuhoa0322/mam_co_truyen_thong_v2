...
CREATE TABLE shopping_items (
  id               INTEGER PRIMARY KEY AUTOINCREMENT,
  list_name        TEXT    NOT NULL DEFAULT 'Danh sách đi chợ',
  list_occasion    TEXT,           -- 'Tết Giáp Thìn', 'Giỗ Bà'

  dish_id          INTEGER REFERENCES dishes(id) ON DELETE SET NULL,
  dish_name        TEXT,           -- Label nhóm: 'Kho Tàu', 'Gói Bánh Chưng'
...
CREATE INDEX idx_shopping_list ON shopping_items(list_name);
...
FROM shopping_items
GROUP BY list_name, list_occasion;
...
INSERT INTO shopping_items
  (list_name, list_occasion, dish_id, dish_name,
   ingredient_name, quantity, unit, estimated_price, actual_price, is_checked, notes)
VALUES
  -- Từ droplist chọn Thịt Kho Tàu (dish_id = 1)
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', 1, 'Kho Tàu',
   'Thịt Ba Chỉ',    1, 'kg',    150000, NULL,   0, NULL),
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', 1, 'Kho Tàu',
   'Trứng Vịt',      6, 'quả',    30000, NULL,   0, NULL),
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', 1, 'Kho Tàu',
   'Nước Dừa Tươi',  2, 'trái',   20000, NULL,   0, NULL),

  -- Từ droplist chọn Bánh Chưng (dish_id = 2)
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', 2, 'Gói Bánh Chưng',
   'Lá Dong',        5, 'bó',     50000, NULL,   0, NULL),
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', 2, 'Gói Bánh Chưng',
   'Gạo Nếp',        1, 'kg',     40000, NULL,   0, NULL),

  -- Đã mua: is_checked=1, actual_price có giá → cộng vào spent_amount
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', 1, 'Ăn kèm',
   'Hành Muối',      1, 'hũ',     45000, 45000,  1, NULL),

  -- Nhập tay: dish_id = NULL
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', NULL, NULL,
   'Dầu Ăn',         2, 'L',      90000, 88000,  1, 'Neptune Gold'),
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', NULL, 'Ban thờ',
   'Nhang Trầm',     1, 'hộp',   200000, 195000, 1, 'Loại thượng hạng'),
  ('Danh Sách Di Chợ', 'Tết Giáp Thìn', NULL, NULL,
   'Mứt Tết Thập Cẩm', 1, 'kg', 120000, NULL,   0, 'Hữu Nghị');
...
-- FROM v_shopping_summary WHERE list_name = 'Danh Sách Di Chợ';

-- Screen 4: Danh sách items nhóm theo món
-- SELECT id, ingredient_name, dish_name, quantity, unit,
--        estimated_price, actual_price, is_checked
-- FROM shopping_items WHERE list_name = 'Danh Sách Di Chợ'
